...
function install(opts, cb) {
  ...
  if (opts.drivers) {
    var includeDefaults = opts.includeDefaultDrivers !== undefined ? opts.includeDefaultDrivers : defaultConfig.includeDefaultDrivers
    if (includeDefaults) {
  ...
  var driversToInstall = Object.keys(opts.drivers);
  logInstallSummary(logger, driversToInstall, fsPaths, urls);
...
function logInstallSummary(logger, drivers, paths, urls) {
  ['selenium'].concat(drivers).forEach(function log(name) {
    if (!paths[name]) {
      return;
    }
